...
public class Dialog implements Closeable {
    ...
    Dialog(MyScreen screen, Root root) {
        this.screen = screen;
        this.root = root;
    }
    ...
    public Dialog useShade() {
        ...
        _shade.setDepth(screen.dialogDepth - 1);
        return this;
    }
    ...
    public Dialog display(float packWidth) {
        ...
        root.layer.setDepth(screen.dialogDepth);
        ...
        screen.dialogDepth += 2; // ensure that subsequent dialogs will render above this one
    ...
    public void dismiss() {
        ...
        _anim.hide(screen, root, _duration).then().action(new Runnable() {
            public void run() {
                ...
                screen.dialogs.remove(Dialog.this);
                close();
            }
    ...
    public void close() {
        if (screen.iface.disposeRoot(root)) {
            screen.dialogDepth -= 2; // reduce dialog depth now that we're gone
    ...
    enum DialogAnim {
        NONE {
            @Override
            public Animation show(UIScreen screen, Root root, float duration, float y) {
                root.layer.setTranslation((screen.size().width() - root.size().width()) / 2, y);
                return screen.iface.anim.delay(0);
            }

            @Override
            public Animation hide(UIScreen screen, Root root, float duration) {
                return screen.iface.anim.delay(0);
            }
        },
        SLIDE_LEFT_RIGHT {
        ...
        GROW_SHRINK {
            @Override
            public Animation show(final UIScreen screen, final Root root, float duration,
                                  final float y) {
        ...
        public abstract Animation show(UIScreen screen, Root root, float duration, float y);

        public abstract Animation hide(UIScreen screen, Root root, float duration);
    }
